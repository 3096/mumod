...
public static class Shim
{
    ...
    private static readonly Func<int> GetKaleidxScopeGateId = GameInfo.GameVersion is >= 25000 and < 26500 ? () => 
    {
        var type = System.Type.GetType("Manager.KaleidxScopeManager, Assembly-CSharp");
        if (type == null) return 0;
        var instance = typeof(MAI2.Util.Singleton<>).MakeGenericType(type).GetProperty("Instance").GetValue(null, null);
        return Traverse.Create(instance).Field<int>("gateId").Value;
    } : () => 0;
